...
const vitalNextPath = 'packages/b1';
const vitalPrefix = '__vital__';
const vitalNextPrefix = 'b1';
...
if (vitalHash !== vitalNextHash) {
  ...
  // Log file missing from b1
  vitalMoreFiles.forEach(file => differences.push(
    `${RED}\t--vital-- contains the file ${file}, it is missing from b1.${NC}`
  ));

  // Log file missing from --vital--
  vitalNextMoreFiles.forEach(file => differences.push(
    `${RED}\tb1 contains the file ${file}, it is missing from --vital--${NC}`
  ));
  ...
  if (differences.length === 0) {
    console.error(`${GREEN}The packages __vital__ and b1 have the same code structure.${NC}`);
    process.exit(0);
  }

  console.error(`${RED}The packages __vital__ and b1 differ in these files:${NC}\n`);
  differences.forEach(difference => console.error(difference));
  process.exit(-1);
}

console.error(`${GREEN}The packages __vital__ and b1 are identical.${NC}`);
